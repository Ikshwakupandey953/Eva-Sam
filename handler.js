...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.sock = conn
        global.img = 'https://telegra.ph/file/e32f2651bac64b6d62c81.jpg'
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (plugin.level > _user.level) {
                    	sock.sendButton(m.chat, `Level required*${plugin.level}* to use this command. Your level*${_user.level},* raise your level by typing *${usedPrefix}levelup* or click the button below!`, wm, null, [[`Levelup`, `#levelup`]], m)
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            sock.sendButtonLoc(jid, text, wm, await(await fetch(pp)).buffer(), [[action === 'add' ? 'Welcome🙏' : 'Goodbye👋', 'Eva']], null, {
                                contextInfo: {
                                    mentionedJid: [user]
                                }
                            })
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used by  _*Owner!*',
        owner: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used by  _*Owner!*',
        mods: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used by  _*Moderator!*',
        premium: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used by  _*Premium User!*',
        group: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used in  _*Groups!*',
        private: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used in  _*Private Chat!*',
        admin: '*────「 ACCESS DENIED 」────*\n\n\nThis command can only be used by  _*Admin!*',
        botAdmin: '*────「 ACCESS DENIED 」────*\n\n\nMake the bot the _*Admin*_ to use this command',
        unreg: '*────「 ACCESS DENIED 」────*\n\n\nPlease register to use this feature by typing:\n*#register <name>.<age>*\n\nExample: *#register Eva.18*',
        nsfw: '*────「 ACCESS DENIED 」────*\n\n\nNSFW doesn\'t active!.',
        rpg: '*────「 ACCESS DENIED 」────*\n\n\nRPG doesn\'t active.'
      }[type]
